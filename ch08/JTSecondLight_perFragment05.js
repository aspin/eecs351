...
var FSHADER_SOURCE =
  ...
  'struct Lamp { ' +
  ' vec3 u_LampPos, u_LampAmb, u_LampDiff, u_LampSpec; ' +
  ' vec3 u_Ke, u_Ka, u_Ks;' +
  ' int u_Kshiny; ' +
  '};' +
  'uniform Lamp lamp0;' +
...
//	'  vec3 normal = v_Normal; \n' +
  ...
  '  vec3 eyeDirection = normalize(u_eyePosWorld - v_Position.xyz); \n' +
	...
	'  float e64 = pow(nDotH, float(lamp0.u_Kshiny));\n' +
/*
...
*/
  ...
  '	 vec3 emissive = lamp0.u_Ke;' +
  '  vec3 ambient = lamp0.u_LampAmb * lamp0.u_Ka;\n' +
  '  vec3 diffuse = lamp0.u_LampDiff * v_Kd * nDotL;\n' +
  '	 vec3 speculr = lamp0.u_LampSpec * lamp0.u_Ks * e64;\n' +
...
var lamp0 = false;
function Lamp() {
  this.uLoc_LampPos = false;
  this.uLoc_LampAmbi = false;
  this.uLoc_LampDiff = false;
  this.uLoc_LampSpec = false;
  ...
  this.lamp0Diff = new Float32Array(3);	// r,g,b for diffuse illumination
  this.lamp0Spec = new Float32Array(3);	// r,g,b for specular illumination
  ...
  this.matl_Kshiny = false;										// specular shinyness exponent.
}

Lamp.prototype.initLamp = function (posGLVar, ambGLVar, diffGLVar, specGLVar) {
  //  ... for Phong light source:
  this.uLoc_LampPos = gl.getUniformLocation(gl.program, posGLVar);
  this.uLoc_LampAmbi = gl.getUniformLocation(gl.program, ambGLVar);
  this.uLoc_LampDiff = gl.getUniformLocation(gl.program, diffGLVar);
  this.uLoc_LampSpec = gl.getUniformLocation(gl.program, specGLVar);
  if (!this.uLoc_LampPos || !this.uLoc_LampAmbi || !this.uLoc_LampDiff || !this.uLoc_LampSpec) {
    console.log('Failed to get the Lamp0 storage locations');
  }
};

Lamp.prototype.initPhongGL =  function (keGLVar, kaGLVar, kdGlVar, ksGlVar, kShinyGlVar) {
  // ... for Phong material/reflectance:
  this.uLoc_Ke = gl.getUniformLocation(gl.program, keGLVar);
  this.uLoc_Ka = gl.getUniformLocation(gl.program, kaGLVar);
  this.uLoc_Kd = gl.getUniformLocation(gl.program, kdGlVar);
  this.uLoc_Ks = gl.getUniformLocation(gl.program, ksGlVar);
  this.uLoc_Kshiny = gl.getUniformLocation(gl.program, kShinyGlVar);
...
Lamp.prototype.initWorldReflect =  function (o) {
  // Init World-coord. position & colors of first light source in global vars;
  this.lampPos.set(o.pos);
  this.lampAmbi.set(o.amb);
  this.lamp0Diff.set(o.diff);
  this.lamp0Spec.set(o.spec);
  // then use those global vars to set 'uniform' vars values inside the GPU:
  gl.uniform3fv(this.uLoc_LampPos, this.lampPos);
  gl.uniform3fv(this.uLoc_LampAmbi, this.lampAmbi);		// ambient
  gl.uniform3fv(this.uLoc_LampDiff, this.lamp0Diff);		// diffuse
  gl.uniform3fv(this.uLoc_LampSpec, this.lamp0Spec);		// Specular
  // Init reflectance values for our (first) Phong material in global variables;
  this.matl_Ke.set(o.ke);
  this.matl_Ka.set(o.ka);
  this.matl_Kd.set(o.kd);
  this.matl_Ks.set(o.ks);
  this.matl_Kshiny = o.kshiny;													// Specular shinyness exponent
  // then use those global vars to set 'uniform' vars in the GPU:
  gl.uniform3fv(this.uLoc_Ke, this.matl_Ke);				// Ke emissive
  gl.uniform3fv(this.uLoc_Ka, this.matl_Ka);				// Ka ambient
  gl.uniform3fv(this.uLoc_Kd, this.matl_Kd);				// Kd	diffuse
  gl.uniform3fv(this.uLoc_Ks, this.matl_Ks);				// Ks specular
  gl.uniform1i(this.uLoc_Kshiny, this.matl_Kshiny);					// Kshiny shinyness exponent
};

Lamp.prototype.setLightSource = function (valArr) {
  // NEW!  re-set the light-source global vars to its original values:
  this.lampPos.set(valArr);
  // then use those global vars to set 'uniform' vars values inside the GPU:
  gl.uniform3fv(this.uLoc_LampPos, this.lampPos);
};

Lamp.prototype.setXYZ = function (arrXYZ) {
  this.lampPos.set(arrXYZ);	// Vert drag*4 changes world z
...
Lamp.prototype.increaseShiny = function () {
  this.matl_Kshiny += 1;								// INCREASE shinyness, but with a
  if (this.matl_Kshiny > 128) this.matl_Kshiny = 128;	// upper limit.
  console.log('UPPERcase S: ++K_shiny ==', this.matl_Kshiny, '\n');
  gl.uniform1i(this.uLoc_Kshiny, this.matl_Kshiny);		// Set new value of Kshiny uniform
};

Lamp.prototype.decreaseShiny = function () {
  this.matl_Kshiny -= 1;										// DECREASE shinyness, but with a
  if (this.matl_Kshiny < 1) this.matl_Kshiny = 1;		// lower limit.
  console.log('lowercase s: --K_shiny ==', this.matl_Kshiny, '\n');
  gl.uniform1i(this.uLoc_Kshiny, this.matl_Kshiny);		// Set new value of Kshiny uniform
};

Lamp.prototype.getLampPos = function () {
  return this.lampPos;
};
...
function main() {
  ...
  lamp0.initLamp('lamp0.u_LampPos', 'lamp0.u_LampAmb', 'lamp0.u_LampDiff', 'lamp0.u_LampSpec');
  lamp0.initPhongGL('lamp0.u_Ke', 'lamp0.u_Ka', 'u_Kd', 'lamp0.u_Ks', 'lamp0.u_Kshiny');
...
// Called when user presses 'Clear' button in our webpage
  ...
  lamp0.setLightSource( [6.0, 5.0, 5.0]);
...
//Mouse-Drag Moves Lamp0 ========================================================
  ...
  lamp0Pos = lamp0.getLampPos();
	lamp0.setXYZ([lamp0Pos[0],										// don't change world x;
